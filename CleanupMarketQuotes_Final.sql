...
-- First, drop the primary key constraint on Id column
ALTER TABLE MarketQuotes DROP CONSTRAINT PK_MarketQuotes;
GO
...
-- Make BusinessDate NOT NULL (update any NULL values first)
...
-- Make InsertionSequence NOT NULL (update any NULL values first)
...
-- Create a new primary key on a combination of columns that uniquely identify a record
ALTER TABLE MarketQuotes ADD CONSTRAINT PK_MarketQuotes PRIMARY KEY (BusinessDate, TradingSymbol, InsertionSequence);

PRINT 'MarketQuotes table cleanup completed - removed 47 unwanted columns and created new primary key';
